...
public class UIAutoAnimation : MonoBehaviour
{
    ...
    public float GetMaxEntranceDuration()
    {
        SOAnimationPresets animationPresets = animationEntrancePresets;

        float alphaDuration = (!animationPresets.useAlphaAnimation) ? 0 : (componentList.Count * animationPresets.delayPerElementAlpha) + animationPresets.alphaDuration;
        float positionDuration = (!animationPresets.usePositionAnimation) ? 0 : (componentList.Count * animationPresets.delayPerElementPosition) + animationPresets.positionDuration;
        float scaleDuration = (!animationPresets.useScaleAnimation) ? 0 : (componentList.Count * animationPresets.delayPerElementScale) + animationPresets.scaleDuration;
        float rotationDuration = (!animationPresets.useRotationAnimation) ? 0 : (componentList.Count * animationPresets.delayPerElementRotation) + animationPresets.rotationDuration;
        float maxEntranceDuration = Mathf.Max(alphaDuration, positionDuration, scaleDuration, rotationDuration);

        return maxEntranceDuration;
    }

    public float GetMaxExitDuration()
    {
        SOAnimationPresets animationPresets = animationExitPresets;
